...
from neon.backends.backend import Backend, Block
...
class MGPU(GPU):
    ...
    def begin_stack(self, block, identifier):
        if block == Block.fprop:
            self.ng.block = Block.fprop
            self.call_stack = []
        else:
            pass
    ...
    def fprop_fc(self, out, inputs, weights, layer=None):
        ...
        for dbuf, ibuf, wt, bs, strm, ctx in zip(ubuf.tlist, inputs.tlist,
             weights.tlist, biases, self.strms, self.ctxs):
            ctx.push()
            self.ng.stream = strm
    ...
    def bprop_fc(self, out, weights, deltas, layer=None):
        ...
        for dbuf, ibuf, wt, strm, ctx, off in zip(
             out.tlist, deltas.tlist, weights.tlist, self.strms, self.ctxs,
             starts):
            ctx.push()
            self.ng.stream = strm
    ...
    def update_fc(self, out, inputs, deltas, layer=None):
        ...
        for obuf, dbuf, ibuf, strm, ctx, off in zip(
             out.tlist, deltas.tlist, inputs.tlist, self.strms, self.ctxs,
             starts):
            ctx.push()
            self.ng.stream = strm
    ...
    def update_fc_bias(self, err, out):
        ...
        for ebuf, obuf, strm, ctx, off in zip(
             err.tlist, out.tlist, self.strms, self.ctxs, starts):
            ctx.push()
            self.ng.stream = strm
